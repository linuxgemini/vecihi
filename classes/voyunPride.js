...
class voyunPride extends DiscordJSClient {
    constructor(djsOptions = {}) {
        ...
        if (this.ayarlar.tmiConfig.identity.username === "") throw new Error("Config file has missing data on tmiConfig.");

        this.tmijs = TMI.client(this.ayarlar.tmiConfig),
        this.isOnBanningStreak = false,
    ...
    async ucagiKaldir() {
        ...
        this.isBotConstructed && await this.login(this.ayarlar.anahtar);
        this.isBotConstructed && await this.tmijs.connect();
        return null;
    }
